...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // พยายามลบทั้งโฟลเดอร์
    const folder = inferNewsFolderFromCover(deleted.image);
    if (folder) {
      const absFolder = path.join(process.cwd(), "uploads", "news", folder);
      if (fs.existsSync(absFolder)) {
        try { fs.rmSync(absFolder, { recursive: true, force: true }); } catch { }
      }
    } else {
      // fallback: ลบทีละไฟล์จาก contentHtml
