  ...
  const copy = async () => {
    try {
      if (navigator.clipboard?.writeText) {
        ...
        // fallback สำหรับ environment ที่ปิด clipboard API
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56">
        ...
        <DropdownMenuItem onClick={() => open(lineShare)}>
          {/* ไอคอน Line แบบ SVG เบาๆ */}
          <svg viewBox="0 0 36 36" className="mr-2 h-4 w-4" aria-hidden="true">
            <path d="M30 16.2c0-5.52-5.37-10-12-10S6 10.68 6 16.2C6 22 11.38 26 18 26c.74 0 1.46-.05 2.16-.15.77.45 2.69 1.55 4.17 2 .4.12.76-.26.64-.65-.3-1.05-.58-2.28-.46-2.94 3.4-1.82 5.49-4.85 5.49-8.06Z" />
          </svg>
          Line
        </DropdownMenuItem>
        <DropdownMenuItem
          ...
          (window.location.href = `mailto:?subject=${encodeURIComponent(
            "แชร์ข่าวสาร",
          )}&body=${encodeURIComponent(`ดูข้อมูลเพิ่มเติมได้ที่: ${url}`)}`)
          }
        >
